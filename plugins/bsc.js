  ...
  const bsc = new Vue({
    ...
    methods: {
      ...
      checkBscFormat (bscAddress) {
        const response = web3.utils.isAddress(bscAddress, 97)
        return response
      },
      ...
      async onWalletConnectWeb3 () {
        try {
          ...
          // this.updateAccount()
          this.walletConnect.updateRpcUrl(97, 'https://bsc-dataseed.binance.org')
      ...
      async onCorrectChain () {
        try {
          ...
          const bool = Boolean(currentChain === '0x61')
          return bool
        } catch (error) {
      ...
      registerProviderListener (provider) {
        ...
        this.currentProvider.on('chainChanged', (_chainId) => {
          if (_chainId !== '0x61') {
            alert('Please switch to the correct chain: Binance Smart Chain, Mainnet, chainId: 97')
          }
        })
